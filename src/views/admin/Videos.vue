      ...
      <div v-loading="dialogLoading">
        <div class="video-info" v-if="currentVideo">
          <el-descriptions :column="1" border>
            ...
            <el-descriptions-item label="上传时间">{{ formatDate(currentVideo.createTime) }}</el-descriptions-item>
...
const handleDelete = (video) => {
  ElMessageBox.confirm(
    ...
    .then(async () => {
      try {
        const res = await deleteVideoService(video.id)
        if (res.code === 200) {
...
const handleView = async (video) => {
  ...
  try {
    const res = await getVideoByIdService(video.id)
    if (res.code === 200) {
